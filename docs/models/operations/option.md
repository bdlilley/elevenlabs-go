...
```go
elevenlabsgo.WithServerURL("https://api.example.com")
```
...
```go
elevenlabsgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
elevenlabsgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
elevenlabsgo.WithTimeout(30 * time.Second)
```
